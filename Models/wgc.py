...
import Utils.wgnUtils as layers
import numpy as np
...
class GraphConvLSTM(object):
    ...
    def __init__(self, adj, n_stations, n_features, num_layers=1, n_steps=14 * 4,
                 n_hidden=16, adj_trainable=True, use_sparse=False, mask_len=0,
    ...
    def _build(self):
        ...
        builds the model
        '''
        self.inputs_placeholder = tf.compat.v1.placeholder(tf.float32,
                                                           [None, self.n_steps, self.n_stations, self.n_features],
                                                           name='x')
        self.outputs_placeholder = tf.compat.v1.placeholder(tf.float32,
                                                            [None, self.n_steps-self.mask_len, self.n_stations],
                                                            name='y')
        ...
        with tf.name_scope('adjacency'):
            ...
            if self.rank:
                self.factor1 = tf.compat.v1.get_variable("factor1", [self.n_stations, self.rank])
                self.factor2 = tf.compat.v1.get_variable("factor2", [self.rank, self.n_stations])
            ...
            # Check to see if we use a sparse or dense representation of the adjacency matrix.
                ...
                self.adj_var = tf.Variable(vals, dtype=tf.float32, trainable=self.adj_trainable,
                                           name='adjacency_variable')
                self.adj_tensor = tf.SparseTensor(indices, self.adj_var, self.adj.shape)
            else:  # use a dense representation of adj
                self.adj_var = tf.Variable(self.adj, dtype=tf.float32, trainable=self.adj_trainable,
                                           name='adjacency_variable_tensor')
                self.adj_tensor = self.adj_var

        self.w_pred = tf.Variable(tf.compat.v1.random_normal([self.n_hidden, 1]), name='weight')
        self.b_pred = tf.Variable(tf.compat.v1.random_normal([1]), name='bias')
        ...
        for l in range(self.num_layers):
            state = None
            with tf.compat.v1.variable_scope("lstm_layer-" + str(l)) as scope:  # as BasicLSTMCell
        ...
        with tf.name_scope('loss'):
            self.mse = tf.reduce_mean(tf.compat.v1.squared_difference(self.outputs_placeholder, self.preds), name='mse')
        ...
        with tf.name_scope('optimization'):
            self.opt_op = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate_variable).minimize(
                self.loss, global_step=self.global_step, name='optimizer')
...
def _gconv(args, output_size, adj, stddev=0.001, bias=True, bias_start=0.0):
    ...
    kernel = vs.get_variable("kernel", [total_arg_size, output_size],
